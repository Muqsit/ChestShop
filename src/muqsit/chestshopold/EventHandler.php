	...
	public function handleTransaction(Player $player, Item $itemClicked, Item $itemClickedUsing, SlotChangeAction $inventoryAction) : bool{
			...
			switch($nbt->getByte("Button")){
				case Button::TURN_LEFT:
					...
					if($this->currentpage[$playerId] === false){
						...
						$player->sendMessage(TextFormat::RED . "Could not find category '" . $category . "', perhaps it has been removed.");
					}
					break;
		...
		if($nbt->hasTag("ChestShop")){
			...
			if($this->economy->myMoney($player) < $cost){
				$player->sendMessage(TextFormat::RED . "You do not have enough money to purchase " . $itemClicked->getName() . ".");
				$player->removeWindow($inventoryAction->getInventory());
				return true;
			}

			if($this->doubleclicks !== null){
				if(!isset($this->doubleclicks[$playerId = $player->getId()]) || $player->ticksLived - $this->doubleclicks[$playerId] > 15//15 tick delay for double taps, probably make this a configurable number
				){
			...
			$player->sendMessage(TextFormat::YELLOW . "Purchased " . $itemClicked->getName() . " (x" . $itemClicked->getCount() . ")" . TextFormat::RESET . TextFormat::YELLOW . " for \$" . TextFormat::RESET . TextFormat::GOLD . $cost);
