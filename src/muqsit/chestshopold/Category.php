...
class Category{
	...
	public function __construct(ChestShop $plugin, string $name, Item $identifier){
		...
		$this->menu->readonly()->setName($name)->setListener([$plugin->getEventHandler(), "handleTransaction"])->setInventoryCloseListener([$plugin->getEventHandler(), "handlePageCacheRemoval"]);
	}
	...
	public function getIdentifier() : Item{
		...
		$item->setCustomName(TextFormat::RESET . TextFormat::BOLD . TextFormat::AQUA . $this->name);
		$item->setLore([TextFormat::RESET . TextFormat::GRAY . "Click to view this category."]);
	...
	public function addItem(Item $item, float $cost) : void{
		...
		$lore = $item->getLore();
		$lore[] = TextFormat::RESET . TextFormat::YELLOW . TextFormat::BOLD . "COST: \$" . TextFormat::RESET . TextFormat::GOLD . sprintf("%.2f", $cost);
		$item->setLore($lore);
	...
	public function send(Player $player, int $page = 1, bool $send = true) : int{
		if(!empty($this->items)){
			...
			$page = min((int) ceil(count($this->items) / 45), max(1, $page));//page > 0 and page <= number of pages.
